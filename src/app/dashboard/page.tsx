  ...
  const AIAssistantSection = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        setError(error.message || 'Er is een fout opgetreden bij het versturen van uw bericht.');
          ...
          <div className="space-y-4">
            <div className="border rounded-lg p-4 h-96 overflow-y-auto">
              {messages.map((message, index) => (
                <div key={index} className={`mb-8 ${message.role === 'assistant' ? 'bg-blue-50 p-4 rounded-lg' : ''}`}>
                  <div className="flex items-start space-x-3 mb-2">
                    <div className="flex-shrink-0 font-semibold text-blue-600">
                      {message.role === 'user' ? 'U:' : 'AI Assistant:'}
                    </div>
                  </div>
                  <div className="flex-grow whitespace-pre-wrap ml-8">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div>
              <span className="text-gray-600">
                Welkom terug, {currentSession.user?.email}
              </span>
